...
- 文件：
  ...
  - `src/api/types.ts` - API类型定义（包含 UserRole 类型）
  - `src/api/auth.ts` - 认证API封装
  - `src/api/imageTag.ts` - 图片标签API封装
  - `src/api/image.ts` - 图片管理API封装
- 功能：
  - 自动携带JWT token（localStorage key: `auth_token`）
  - 401自动跳转登录
  - 统一响应处理（code/message/data）
- 配置规范：
  - **baseURL 包含 `/api` 前缀**（推荐做法）
  - 开发环境：`http://127.0.0.1:3000/api`
  - 生产环境：`https://api.mumumumu.net/api`
  - API 调用路径**不需要**写 `/api` 前缀
  - 示例：`request.get('/auth/login')` → 实际请求 `/api/auth/login`
- 注意事项：
...
- 关键实现：
  ...
  - AdminRoute 检查 `user.role === UserRole.ADMIN`
  - 解决刷新页面丢失登录状态的问题
- **状态**：已完成，禁止修改
...
### ✅ 管理后台
- 文件：
  - `src/pages/Super/index.tsx` - 管理后台首页
  - `src/pages/Super/ImageManage/index.tsx` - 图片管理页面
- 功能：
  - **管理首页** (`/super`)：功能卡片展示，导航到各管理模块
...
- 公开页面：`/`、`/login`、`/register`
- 需要登录：`/dashboard`、`/tools/*`（使用 `<PrivateRoute>`）
- 仅管理员：`/super`、`/super/*`（使用 `<AdminRoute>`）

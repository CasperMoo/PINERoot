...
**敏感信息示例**：
- ❌ `userId` - 用户ID（可能被用于用户画像或攻击）
- ❌ `ossKey` - OSS 内部存储路径（可能被滥用或暴力枚举）
- ❌ `size`, `width`, `height`, `mimeType` - 文件元数据
- ❌ `tagId` - 内部标签ID（暴露数据库结构）
- ❌ `deletedAt`, `createdAt`, `updatedAt` - 内部时间戳
- ❌ `tag.id` - 标签的内部ID
...
**实现示例**（参考 `src/services/anchor/halloween.ts`）：
...
export async function getPublicImages() {
  const result = await getImageList({ ... })
  return result  // 包含 userId, ossKey 等敏感信息
}
```
...
**适用场景**：
- ✅ 所有无需登录的公开接口（如 `/api/anchor/halloween/images`）
- ✅ 第三方 API 集成接口
- ✅ 对外开放的数据展示接口
- ⚠️  认证用户接口根据业务需求决定（遵循最小权限原则）
